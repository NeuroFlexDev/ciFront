...
const TextEditor: React.FC<TextEditorProps> = ({ value, onChange }) => {
  ...
  const editor = useEditor({
    ...
    content: value,
    onUpdate: ({ editor }) => {
      // 1) Когда пользователь что-то меняет — вызываем onChange
      onChange(editor.getHTML())

      // 2) Обновляем локальные стейты формат/цвет/выравнивание
  ...
  // 3) ВАЖНО: Если prop value меняется (например, при переключении уроков),
  // обновляем контент редактора, если он отличается
  React.useEffect(() => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.toolbar}>
        ...
        {/* Выравнивание */}
        <div className={styles.dropdownContainer}>
          <button className={styles.button} onClick={() => toggleDropdown("align")}>
            <img
          ...
          {showAlign && (
            <div className={styles.dropdown}>
              {textAlignments.map((align) => (
                ...
                >
                  <img src={align.icon} alt={align.alt} className={styles.iconAlign} />
                </div>
              ))}
            </div>
          )}
        ...
        <div className={styles.buttonGroup}>
          {["BOLD", "ITALIC", "UNDERLINE", "STRIKETHROUGH", "CODE"].map((style) => (
            <button
              key={style}
              className={`${styles.button} ${editor.isActive(style.toLowerCase()) ? styles.active : ""}`}
        ...
        <div className={styles.buttonGroup}>
          {["unordered-list-item", "ordered-list-item", "blockquote"].map((type) => (
            <button
              key={type}
              className={`${styles.button} ${editor.isActive(type) ? styles.active : ""}`}
        ...
        <div className={styles.buttonGroup}>
          ...
          <button className={styles.button} onClick={removeLink}>
            Убрать ссылку
          </button>
        </div>
      </div>
